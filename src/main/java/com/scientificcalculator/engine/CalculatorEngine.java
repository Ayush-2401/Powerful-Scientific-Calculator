...
public class CalculatorEngine {
    ...
    public String solveCubic(double coeffA, double coeffB, double coeffC, double coeffD) {
        ...
        return "x₁ = " + formatComplex(roots[0]) + ", x₂ = " + formatComplex(roots[1]) + ", x₃ = " + formatComplex(roots[2]);
    }
     ...
     *
     * @param coeffs The coefficients of the polynomial, from the highest degree to the constant term.
    ...
    public String solvePolynomial(double[] coeffs) {
        ...
        switch (effectiveDegree) {
            ...
            case 4: // Quartic
                return solveQuartic(effectiveCoeffs[0], effectiveCoeffs[1], effectiveCoeffs[2], effectiveCoeffs[3], effectiveCoeffs[4]);
    ...
    /**
     * Implements the Durand-Kerner method to find all roots of a polynomial simultaneously.
    ...
    public Map<String, Double> solveNonLinearSystem(List<String> equations, List<String> variables, double[] initialGuesses) {
        if (equations.size() != variables.size()) {
            throw new IllegalArgumentException("The number of equations must match the number of variables.");
        }
        if (initialGuesses.length != variables.size()) {
            throw new IllegalArgumentException("The number of initial guesses must match the number of variables.");
        }

        List<String> processedEquations = new ArrayList<>();
        for (String eq : equations) {
            if (eq.contains("=")) {
                String[] parts = eq.split("=", 2);
                processedEquations.add(parts[0] + " - (" + parts[1] + ")");
            } else {
                processedEquations.add(eq);
            }
        }

        int n = variables.size();
        double[] x = initialGuesses;

        int maxIterations = 100;
        double tolerance = 1e-7;

        for (int k = 0; k < maxIterations; k++) {
            // Calculate Jacobian matrix J and function vector F
            double[][] J = new double[n][n];
            double[] F = new double[n];

            for (int i = 0; i < n; i++) {
                final int eqIndex = i;
                Function<double[], Double> func = (double[] vals) -> {
                    List<String> tokens = tokenizeExpression(processedEquations.get(eqIndex));
                    Map<String, Double> variablesMap = new LinkedHashMap<>();
                    for (int v = 0; v < variables.size(); v++) {
                        variablesMap.put(variables.get(v), vals[v]);
                    }
                    List<String> postfix = InfixToPostfixConverter.convert(tokens, variables);
                    PostfixEvaluator evaluator = new PostfixEvaluator(isDegreesMode);
                    return evaluator.evaluate(postfix, variablesMap);
                };

                F[i] = -func.apply(x);

                for (int j = 0; j < n; j++) {
                    J[i][j] = numericalPartialDerivative(func, x, j);
                }
            }

            // Solve the linear system J * delta = F
            try {
                Map<String, Double> deltaMap = solveLinearSystem(J, F);
                double[] delta = new double[n];
                for (int i = 0; i < n; i++) {
                    delta[i] = deltaMap.get("x" + (i + 1));
                }

                // Update the solution vector: x = x + delta
                for (int i = 0; i < n; i++) {
                    x[i] += delta[i];
                }
                ...
                if (Math.sqrt(norm) < tolerance) {
                    ...
                    for (int i = 0; i < n; i++) {
                        solution.put(variables.get(i), x[i]);
                    }
                    return solution;
                }
            } catch (ArithmeticException e) {
                throw new ArithmeticException("Could not solve the system. The Jacobian matrix may be singular.");
            }
        }

        throw new ArithmeticException("The solver failed to converge within the maximum number of iterations.");
    }

    private double numericalPartialDerivative(Function<double[], Double> f, double[] x, int varIndex) {
        double h = 1e-7;
        double[] x_h = x.clone();
        x_h[varIndex] += h;

        double f_x = f.apply(x);
        double f_xh = f.apply(x_h);
